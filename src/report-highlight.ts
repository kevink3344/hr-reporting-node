...
export function cellMatches(rule: ReportHighlightRule, cellValue: unknown): boolean {
...
export function normalizeHighlightRules(input: unknown): ReportHighlightRule[] {
  const parsed = reportHighlightRulesSchema.safeParse(input);
  if (!parsed.success) return [];
  return parsed.data as ReportHighlightRule[];
}
